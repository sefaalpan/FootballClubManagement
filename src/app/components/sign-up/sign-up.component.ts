...
export class SignUpComponent implements OnInit {
  ...
  submit() {
    ...
    if (this.registerForm.valid) {
      ...
      let password = md5(this.registerForm.value.email as string);
      let role = this.registerForm.value.role as string;

      let matricule = this.registerForm.value.club as string;
      let club !: Club;
      this.cs.getClubByMatricule(matricule).subscribe(c => club = c);

      if (club) {
        console.log('le club existe deja');

        let president: President = {};
        //le club a-t'il déjà un club
        ...
        if (president !== undefined) {
          ...
          this.router.navigate(['create-club/', this.president.id + '/', matricule]);
